...
`
for name in /home/m280data/NYCHVS/*.csv
do
awk 'END { print NR }' $name
done
`
...
`
> takes the output of one command and plugs it into a file. >> adds the output of one command to a file.
The first one displays 'hello, world' before it is piped into the test1.txt file. The second one directly adds "echo 'hello, world'" to the test2.txt file.
`
...
`
The first line of the shell script tells the loader what type of program to execute the file with. $1 $2 $3 are called positional parameters. The output are the first $2 lines from $1 file, which is piped into the tail command so we so the last $3 lines of the head output.
##chmod 751 filename
`
