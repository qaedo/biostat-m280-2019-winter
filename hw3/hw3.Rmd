...
library(tidyverse)
options(scipen = 999)
employee_1 <- read_csv("/home/m280data/la_payroll/City_Employee_Payroll.csv") %>%
  setNames(str_replace_all(names(.)," ", "_"))
#(employee %<>% select(Year, Department_Title, Job_Class_Title, Total_Payments, 
#                      Base_Pay, Overtime_Pay))
#employee$Other_Pay <- employee$Total_Payments - employee$Base_Pay - employee$Overtime_Pay
#employee.long <- melt(employee, id.vars = c("Year", "Department_Title", "Job_Class_Title"))
...
as.data.frame(total_data)
total_long <- melt(total_data, id.vars = "Year")
saveRDS(total_long, file = "Question2Total.rds")
```
...
total_data_2 <- employee %>%
  group_by(Year, Department_Title) %>%
  summarise(totalPay = sum(Total_Payments, na.rm = TRUE),
            basePay = sum(Base_Pay, na.rm = TRUE),
            overPay = sum(Overtime_Pay, na.rm = TRUE),
            otherPay = sum(Other_Pay, na.rm = TRUE))
as.data.frame(total_data_2)
total_long_2 <- melt(total_data_2, id.vars = c("Year", "Department_Title"))
    Q5 <- total_long_2 %>%
      filter(Year == 2017, variable == "totalPay") %>%
      group_by(Department_Title) %>%
      arrange(desc(value)) %>%
      head(10)
    Q5
    test <- subset(total_long_2, total_long_2$Department_Title %in% Q5$Department_Title & Year == 2017)
    test
#saveRDS(total_long, file = "Question5Total.rds")
```
...
mymonths <- c("Jan","Feb","Mar",
              ...
              "Jul","Aug","Sep",
              "Oct","Nov","Dec")
la_sql$Issue_Month <- mymonths[ la_sql$Issue_Month ]
class(la_sql)
# la_sql %>%
