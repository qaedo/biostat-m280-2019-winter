...
In order to take multiple arguments (numbers), the ellipsis feature was used. Inputs are put into a list object, which is used in a for loop. If an item in the list is not categorized as numeric type, then the function stops. In each loop, the function cycles through 3 conditional statements that utilize the modulo operation. The first conditional tests if the item is a multiple of both 3 and 5; there is a nested conditional that tests if the item is equal to 0, and skips the rest of the loop if true. Otherwise it prints "FizzBuzz" and also skips the next 2 conditionals to loop through the next item. The next conditionals test for only a multiple of 3 or 5 respectively and prints the proper response if true.

```{R}
fizzbuzz <- function(input, ...){
  loopList <- list(input, ...)
  for (integer in loopList){
    if (class(integer) != "numeric")
      stop("Item in input should be a numeric type")
